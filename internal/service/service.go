...
func (s *Service) CreateUser(ctx context.Context, email string) error {
	_, err := s.db.CreateUser(ctx, email)
	if err != nil {
		return fmt.Errorf("s.db.CreateUser(email): %w", err)
	}
...
func (s *Service) CreateWallet(ctx context.Context, owner model.User, currency string) error {
	_, err := s.db.CreateWallet(ctx, owner, currency)
	if err != nil {
		return fmt.Errorf("s.db.CreateWallet(owner, currency): %w", err)
	}
